...
| ------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| 1.0.10  | 2026-08-24 | Fixed cross-device sync reliability: restored light auto-restore polling while a newtab stays visible (visibility-only pulls left long-lived tabs permanently stale), flush the debounced push when the tab hides or unloads, added a persisted syncPending flag so un-pushed edits are merged into the cloud before any pull can overwrite them, and stopped adopting older remote revisions after worker resets. Added controller regression tests.                                                | Draft  |
| 1.0.9   | 2026-08-22 | Replaced fixed-interval auto-restore with visibilitychange-triggered pulls throttled by delaySeconds, deduped sync-status writes to storage.local, and added ISC LICENSE.                                                                                                                                                                                                                                                                                                                            | Draft  |
| 1.0.8   | 2026-08-22 | Fixed two-way sync integrity for group rename/delete: link updatedAt bumps so LWW merge no longer resurrects stale copies, deletedGroupsMap tombstones propagated end-to-end, crypto.randomUUID link ids, and GitHub Actions CI.                                                                                                                                                                                                                                                                     | Draft  |
| 1.0.7   | 2026-08-17 | Root cause of the Android "Service worker registration failed (status code 10)" was the ZIP packaging: PowerShell Compress-Archive stored entry names with backslashes, which Android/Linux extractors treat as literal filename characters, so the extension folder tree was corrupted after extraction. Replaced the packager with a spec-compliant ZIP writer that always uses forward-slash separators, and added build regression tests. Works on Android regardless of browser/install method. | Draft  |
| 1.0.6   | 2026-08-17 | Removed the MV3 background service worker entirely — Chrome for Android cannot reliably register extension service workers ("Service worker registration failed, status code 10 / kErrorNetwork"). Favicon fetching moved in-page; "Add current page" now resolves the last-visited tab via chrome.tabs; toolbar click uses a small popup.                                                                                                                                                           | Draft  |
