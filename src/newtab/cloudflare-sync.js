...
async function loadLocalDefaultWorkerUrl() {
  try {
    ...
    if (typeof config.DEFAULT_WORKER_URL === 'string' && config.DEFAULT_WORKER_URL.trim()) {
      return config.DEFAULT_WORKER_URL.trim();
    }
  } catch {
    // Optional config absent.
  }
  return 'https://extension.quavav15-6.workers.dev';
}
...
export async function loadSavedSyncCredentials(dom) {
  ...
  return new Promise((resolve) => {
    chrome.storage.local.get(
      ...
      (result) => {
        const savedUrl = result[STORAGE_KEYS.syncWorkerUrl];
        const effectiveUrl = savedUrl || localDefaultWorkerUrl;
        dom.syncWorkerUrlInput.value = effectiveUrl;
        if (!savedUrl && effectiveUrl) {
          chrome.storage.local.set({ [STORAGE_KEYS.syncWorkerUrl]: effectiveUrl });
        }
